...
int colorindex(int x, int y) => mod(x + y * 2, 16);
...
void draw(todolist[][] data, int[] xs, int ys)
{
    ...
    foreach (y, list; data)
    {
        foreach (x, e; list)
        {
            e.draw(Vec2((r.x + r.x / 5) * (cast(int) x - xs[ys]), (r.y + r.y / 5) * (cast(int) y - ys)), colorindex(
                    (cast(int) x - xs[ys]), (
                    cast(int) y - ys)));
        }
    }
...
void drawrounded(Rect r, Color outline, Color background)
{
    ...
    rl.DrawRectangleRoundedLinesEx(bk.toRl(r), round, 9, 3, bk.toRl(outline));
}
...
void drawitem(int i, string s, bool crossed, Rect r, int colorindex)
{
    ...
    drawText(textfont, s, where, DrawOptions(color : textcolor));
