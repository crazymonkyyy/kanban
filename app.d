...
bool update(float dt){
	...
	// Use utility.clamptoindex to ensure safe access to both data[y] and x[y]
	// Initialize with an empty column [todolist()] if data is empty
	utility.clamptoindex(data, y, [todolist()]); // Initialize with a default empty column if needed
	...
	if(aiMode) {
		"aimode".writeln;
		takeScreenshot();
		if(screenshotTaken){return true;}
