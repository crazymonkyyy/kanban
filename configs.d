...
string findfont(string font = "")
{
    ...
    if (font[0] == '~' || font[0] == '/' || font[0] == '.')
    { //if it looks like a file, user error if it fails
        exists(font);
        return font;
    }
    string file = standardpath ~ (font.filter!(not!(std.ascii.isWhite))
            .map!(a => std.ascii.toLower(a))).to!string ~ ".ttf";
    if (exists(file))
    {
        if (verbose)
        {
    ...
    if (verbose)
    {
        "file not found, attempting download".writeln;
    }
    string askgoogle = googleapi ~ font.queryformat;
    if (verbose)
    {
        ("trying:" ~ askgoogle).writeln;
    }
    bool found = false;
...
auto queryformat(string s) =>
    s.splitter(' ')
        .filter!(a => a.length > 0)
        .map!(a => [std.ascii.toUpper(a[0])] ~ a[1 .. $].map!(std.ascii.toLower)
                .to!string)
        .joiner("+")
        .to!string;
unittest
